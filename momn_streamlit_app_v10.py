...
from data_service import fetch_data, fetch_ath_weekly
...
def app_content():
    ...
    dates = {
        'startDate': datetime.strptime('2000-01-01', '%Y-%m-%d') if api_source == "YFinance"
                     else datetime.strptime(dt2, '%Y-%m-%d') - relativedelta(months=24),
    ...
    if start_button:
        ...
        # ── ATH Weekly fetch (Upstox / Angel One only) ──────────
        # YFinance: daily data 2000 se hai, ATH wahi se calculate hoga
        # Upstox / Angel One: separate weekly fetch — 10yr, ~520 candles only
        if api_source in ("Upstox", "Angel One"):
            ath_progress_bar = st.progress(0)
            ath_status_text  = st.empty()
            with st.spinner(f"Fetching 10-yr Weekly ATH data from {api_source}…"):
                high_ath_weekly = fetch_ath_weekly(
                    api_source   = api_source,
                    symbols      = symbol,
                    end_date     = dates['endDate'],
                    progress_bar = ath_progress_bar,
                    status_text  = ath_status_text,
                )
        else:
            high_ath_weekly = pd.DataFrame()   # YFinance: daily high use hoga
        ...
        # ATH source:
        #   YFinance   → daily high from 2000 (already in `high`)
        #   Upstox / Angel One → weekly 10yr fetch; fallback to daily if fetch failed
        if api_source == "YFinance":
            _ath_src = high.loc[:dates['endDate']]
        elif not high_ath_weekly.empty:
            _ath_src = high_ath_weekly.reindex(columns=symbol)
        else:
            _ath_src = high.loc[:dates['endDate']]   # fallback
        dfStats['ATH']     = round(_ath_src.max(), 2)
