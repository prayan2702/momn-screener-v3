...
def app_content():
    ...
    if start_button:
        ...
        # ─────────────────────────────────────────────────────────
        # PORTFOLIO REBALANCING — compute first (needed for Excel)
        # ─────────────────────────────────────────────────────────
        filtered = dfStats[dfStats['final_momentum']].sort_values('Rank', ascending=True)
        rank_threshold   = 100 if U == 'AllNSE' else 75
        ...
        rebalance_table = pd.DataFrame(columns=['Sell Stocks', 'Buy Stocks', 'Reason for Exit'])
        rebalance_table.index.name = 'S.No.'
        ...
        with st.spinner("Portfolio Rebalancing… Please wait…"):
            try:
                ...
                st.warning(f"Portfolio data load failed: {e}")

        # ── Failed Downloads DataFrame for Excel ──────────────
        if failed_blank:
            df_failed_excel = pd.DataFrame({
                'S.No.':        range(1, len(failed_blank) + 1),
                'Failed Stock': failed_blank
            }).set_index('S.No.')
        else:
            df_failed_excel = pd.DataFrame(columns=['Failed Stock'])
            df_failed_excel.index.name = 'S.No.'

        # ─────────────────────────────────────────────────────────
        # NEW FORMAT FUNCTIONS — Failed Downloads & Rebalancing
        # ─────────────────────────────────────────────────────────
        def format_simple_sheet(file_name, sheet_name):
            ...
            # Portfolio Rebalancing — Sell=light red, Buy=light green
            if sheet_name == "Portfolio Rebalancing":
                ...
                for r in range(2, ws.max_row + 1):
                    if sell_col:
                        ws.cell(row=r, column=sell_col).fill = sell_fill
                    if buy_col:
                        ws.cell(row=r, column=buy_col).fill = buy_fill

            # Failed Downloads — orange highlight
        ...
        with pd.ExcelWriter(excel_file, engine="openpyxl") as writer:
            ...
            df_failed_excel.to_excel(writer, sheet_name="Failed Downloads",      index=True)
            rebalance_table.to_excel(writer, sheet_name="Portfolio Rebalancing", index=True)
        ...
        # ── Display Rebalancing table on screen ───────────────
